...
HRESULT DX11PhysicsFramework::InitRunTimeData()
{
	...
	GameObject* gameObject = new GameObject("Floor", planeGeometry, noSpecMaterial, 0);
	gameObject->GetTransform()->SetPosition(0.0f, 0.0f, 0.0f);
	gameObject->GetTransform()->SetScale(15.0f, 15.0f, 15.0f);
	gameObject->GetTransform()->SetRotation(360.0f + 90.0f, 0.0f, 0.0f);
	...
	for (auto i = 0; i < 4; i++)
	{
		gameObject = new GameObject("Cube " + i, cubeGeometry, shinyMaterial, d);
		gameObject->GetTransform()->SetScale(1.0f, 1.0f, 1.0f);
		gameObject->GetTransform()->SetPosition(-2.0f + (i * 2.5f), 1.0f, 10.0f);
		gameObject->GetTransform()->SetRotation(15, 15, 15);
		gameObject->GetAppearance()->SetTextureRV(_StoneTextureRV);
		if (t >= 2) { d = 2; }
		_gameObjects.push_back(gameObject);
	}

	gameObject = new GameObject("Donut", herculesGeometry, shinyMaterial, 0);
	gameObject->GetTransform()->SetScale(1.0f, 1.0f, 1.0f);
...
{
	_accumulator += _phyTimer.GetDeltaTime();
	_phyTimer.Tick();
	while (_accumulator >= FPS60)
	{
		...
		{
			/*_gameObjects[1]->GetPhysicsModel()->AddForce(Vector3(50, 0, 0));*/
			_gameObjects[1]->GetPhysicsModel()->AddRelativeForce(Vector3(10, 0, 0), Vector3(0, 1, 0));
		}
		if (GetAsyncKeyState('2'))
		{
			/*_gameObjects[1]->GetPhysicsModel()->AddForce(Vector3(-50, 0, 0));*/
			_gameObjects[1]->GetPhysicsModel()->AddRelativeForce(Vector3(-10.0f, 0, 0), Vector3(0, 0, 1));
		}
		if (GetAsyncKeyState('3'))
		{
			_gameObjects[2]->GetPhysicsModel()->AddForce(Vector3(0, 0, 50.0f));
		}
		...
		{
			_gameObjects[3]->GetPhysicsModel()->AddForce(Vector3(0, 0, 50.0f));
		}
		...
		{
			_gameObjects[4]->GetPhysicsModel()->AddForce(Vector3(50, 0, 0));
		}
		...
		{
			_gameObjects[1]->GetPhysicsModel()->AddRelativeForce(Vector3(0, 50, 0), Vector3(0, 1, 0));
		}
		// Update camera
		...
		if (_gameObjects[3]->GetPhysicsModel()->IsCollideable() && _gameObjects[4]->GetPhysicsModel()->IsCollideable())
		{
			if(_gameObjects[3]->GetPhysicsModel()->GetCollider()->CollidesWith(*_gameObjects[4]->GetPhysicsModel()->GetCollider(), manifold))
...
void DX11PhysicsFramework::ResolveCollisions()
{
	...
	Collider* objectACollider = objectA->GetCollider();
	Collider* objectBCollider = objectB->GetCollider();
	
	Vector3 objectAPosition = objectACollider->GetPosition();
	Vector3 objectBPosition = objectBCollider->GetPosition();


	if (objectA->IsCollideable() && objectB->IsCollideable() && objectACollider->CollidesWith(*objectBCollider, manifold))
	{
		Vector3 collisionNormal = manifold.collisionNormal;
		/*Vector3 collisionNormal = objectACollider->GetPosition() - objectBCollider->GetPosition();*/
		collisionNormal = collisionNormal.Normalize();
		Vector3 relativeVelocity = objectA->GetVelocity() - objectB->GetVelocity();

		if (relativeVelocity.Magnitude() * collisionNormal.Magnitude() > 0.0f)
		{
			float e = 0.2;
			...
			//float depth = (objectAPosition - objectBPosition).Magnitude()
			//	- objectACollider->GetRadius() - objectBCollider->GetRadius();
			float depth = manifold.points[0].penetrationDepth;


			Vector3 addedVector = collisionNormal * depth * invMassA;
			_gameObjects[1]->GetTransform()->SetPosition(objectAPosition + addedVector);

			addedVector = collisionNormal * depth * -invMassB;
			_gameObjects[2]->GetTransform()->SetPosition(objectBPosition + addedVector);

			float restitution = 1;
			float vj = -(1 + restitution) * collisionNormal * relativeVelocity.Normalize();
			...
			//objectA->AddRelativeForce(invMassA * j * collisionNormal, manifold.points[0].Position - objectAPosition);
			//objectB->AddRelativeForce(-invMassB * j * collisionNormal, manifold.points[0].Position - objectBPosition);
